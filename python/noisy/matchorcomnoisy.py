#Reordering for noisy reads without RC which have already been reordered by some algorithm.
...
infile = "chrom22_50x_noisy_noRC_orcomordered.dna"
outfile = "temp.dna"
readlen = 100
no_reads = 17500000
matchlen = 80
...
for k in range(1,no_reads):
	...
	if chunkstart == True:
		for i in range(maxmatch):
			if(distance.hamming(current[:readlen-i],prev[i:])<=thresh1):
				flag = 1
				if i == 0:
				    	rshift = 2
				else:
		...
		if flag == 0:
			for i in range(1,maxmatch):
				if(distance.hamming(current[i:],prev[:(readlen-i)])<=thresh1):
					flag = 1
					rshift = 0
					chunkstart = False
					break
		...
		if rshift == 1:		
			...
			if flag == 0:
				...
				numchunks+=1
if chunkstart == True:
	chunkpos.append([no_read-1,no_reads-1])
else:
	if rshift == True:
