...
do
	...
	## Run Original File
	suffix="sickle_only_assembly"
	tmp_folder=$assembly_output_folder/$suffix
	/usr/bin/time -v python $spades_tool -t 32 --only-assembler -s $prefix".sickle.fastq" -1 $prefix"_1.sickle.fastq" -2 $prefix"_2.sickle.fastq" -o $tmp_folder | tee $log_folder/$suffix".spades_log"
