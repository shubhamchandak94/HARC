...
void writecontig(std::vector<std::array<double,5>> count, std::list<long> &pos, std::list<std::string> &reads, std::list<uint32_t> &order, std::list<char> &RC, std::list<std::string> &quality, std::ofstream& f_seq, std::ofstream& f_pos, std::ofstream& f_noise, std::ofstream& f_noisepos, std::ofstream& f_order, std::ofstream &f_RC, std::ofstream &f_order_N_pe, std::ofstream &f_flag_N, std::ofstream &f_quality, std::ofstream &f_quality_N, uint32_t list_size)
{
	...
	for(;pos_it!=pos.end(); ++pos_it,++reads_it,++order_it,++RC_it,++quality_it)
	{
		...
		// Averaged counts allowed flag
		if(denoise_type=="averaged_counts_only")
		{
			for(long j = 0; j < readlen; j++)
			{
				...
				double overall_ref_counts = 0;
				double overall_alt_counts = 0;
				if(is_alternate){
					overall_ref_counts += count[currentpos+j][_ref_char_id];
					overall_alt_counts += count[currentpos+j][_read_char_id]; 
				}
				double ref_alt_overall_ratio = overall_alt_counts/overall_ref_counts;
				averaged_counts_allowed_flag = (ref_alt_overall_ratio >= alternate_thresh);
			}
		}
		...
		for(long j = 0; j < readlen; j++)
		{
			...
			if(is_alternate){
				...
				else if(denoise_type == "adaptive_denoising")
				{
					...
					_read_thresh = alternate_thresh*quality_to_p_pbar[_read_quality_val];
					allowed_alternate_flag = (ref_alternate_count_ratio >= _read_thresh);   	
				}
				else if(denoise_type == "adaptive_linear_denoising")
				{
					ref_alternate_count_ratio = (double)count[currentpos+j][_read_char_id]/count[currentpos+j][_ref_char_id];
//linear					
					_read_thresh = alternate_thresh + (double)(quality_thresh - alternate_thresh)*_read_quality_val/42.0;
//exponential
//					_read_thresh = exp(log(quality_thresh/alternate_thresh)/42.0*_read_quality_val+ log(alternate_thresh));
					allowed_alternate_flag = (ref_alternate_count_ratio >= _read_thresh);   	
				}
				else if(denoise_type == "only_using_counts")
				{
				...
				{
					if(averaged_counts_allowed_flag) allowed_alternate_flag = true;
				}
				else if(denoise_type == "averaged_counts_and_quality")
				{
					if(averaged_counts_allowed_flag)
					{
						allowed_alternate_flag = (_read_quality_val >= quality_thresh); 
					}
				}
