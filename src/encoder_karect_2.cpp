...
std::string denoise_read(std::list<std::string> &reads, std::list<long> &cumulative_pos, std::list<std::string> &quality, std::list<std::string>::iterator &reads_it, std::list<long>::iterator &cumulative_pos_it, std::list<std::string>::iterator &quality_it, std::string &ref, std::vector<std::array<double,5>> &count)
{
	...
	while(*cum_pos_it <= current_read_pos - readlen + quality_thresh)
	{
		++cum_pos_it;
		++_reads_it;
		++_quality_it;
...
//	long num_reads_current_pos = 1;
	...
	while(true)
	{
		if(cum_pos_it == cumulative_pos.end())
			break;
		if(*cum_pos_it >= current_read_pos + readlen - quality_thresh)
			break;
		...
		long temp_cum_pos = *cum_pos_it;
		int min_distance = string_hamming(*reads_it,*_reads_it,startposa,startposb,matchlen);
		auto min_reads_it = _reads_it;
		auto min_quality_it = _quality_it;
		++_reads_it;
		++_quality_it;
		++cum_pos_it;	
		while(true)
		{
			if(cum_pos_it == cumulative_pos.end())
				break;
			if(*cum_pos_it > temp_cum_pos)
				break;
			int hamming_dist = string_hamming(*reads_it,*_reads_it,startposa,startposb,matchlen);
			if(hamming_dist < min_distance)
			{
				min_distance = hamming_dist;
				min_reads_it = _reads_it;
				min_quality_it = _quality_it;
			}
			++_reads_it;
			++_quality_it;
			++cum_pos_it;	
		...
		read_weight = 1 - sqrt((double)string_hamming(*reads_it,*min_reads_it,startposa,startposb,matchlen)/(matchlen*0.25));
	//	read_weight = 1;
...
//		if(num_reads_current_pos <= quality_thresh || cum_pos_it == cumulative_pos_it)
		...
		{
			read_count[startposa+i][chartolong[(*min_reads_it)[startposb+i]]] += read_weight*one_minus_p[(uint32_t)((*min_quality_it)[startposb+i])-33];
		}
...
//			num_reads_current_pos++;
	...
	for(int j = 0; j < readlen; j++)
		...
		if(read_count[j][chartolong[(*reads_it)[j]]] > alternate_thresh)// || (read_count[j][0]+read_count[j][1]+read_count[j][2]+read_count[j][3]) < quality_thresh)// && (read_count[j][0]+read_count[j][1]+read_count[j][2]+read_count[j][3]) > quality_thresh)// || (uint32_t)(*quality_it)[j]-33 > quality_thresh)
