...
void reorder(std::bitset<2*readlen> *read, bbhashdict *dict)
{
	omp_lock_t *dict_lock = new omp_lock_t [numreads];
	omp_lock_t *read_lock = new omp_lock_t [numreads];
...
//			std::cout<<tid<<":"<<numdone<<"\n";
		...
		{	b = read[current]&mask1[l];
			...
			int i;
			omp_set_lock(&dict_lock[startposidx]);
