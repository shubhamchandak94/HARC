...
std::string denoise_read(std::list<std::string> &reads, std::list<long> &cumulative_pos, std::list<std::string> &quality, std::list<std::string>::iterator &reads_it, std::list<long>::iterator &cumulative_pos_it, std::list<std::string>::iterator &quality_it, std::string &ref, std::vector<std::array<double,5>> &count);
...
int main(int argc, char** argv)
{
	...
	if(argc > 3)
	{
		...
		else if(denoise_type == "adaptive_linear_denoising"){
			alternate_thresh = atof(argv[4]);
			quality_thresh = atof(argv[5]);
		}
		...
		else if(denoise_type == "averaged_counts_only"){
			alternate_thresh = atof(argv[4]);
		}
		...
		else if(denoise_type == "averaged_counts_and_quality"){
			alternate_thresh = atof(argv[4]);
			quality_thresh = atof(argv[5]);
		}
		else{
...
std::string denoise_read(std::list<std::string> &reads, std::list<long> &cumulative_pos, std::list<std::string> &quality, std::list<std::string>::iterator &reads_it, std::list<long>::iterator &cumulative_pos_it, std::list<std::string>::iterator &quality_it, std::string &ref, std::vector<std::array<double,5>> &count)
{
	long current_read_pos = *cumulative_pos_it;
	std::vector<std::array<double,5>> read_count(readlen,{0,0,0,0,0});
	auto cum_pos_it = cumulative_pos.begin();
	auto _reads_it = reads.begin();
	auto _quality_it = quality.begin();
	double read_weight;
	int startposa, startposb, matchlen;//variables for string_hamming
	//find position of first read overlapping with the current read

	while(*cum_pos_it <= current_read_pos - readlen + 50)//quality_thresh)
	{
		++cum_pos_it;
		++_reads_it;
		++_quality_it;
	}
//	long num_reads_current_pos = 1;
	//now for the reads that do overlap with the current read, start accumulating counts
	auto temp_cum_pos_it = cum_pos_it;
	auto temp_reads_it = _reads_it;
	std::vector<double> overlaps;
	double hamming_limit;
	while(true)
	{
		if(temp_cum_pos_it == cumulative_pos.end())
			break;
		if(*temp_cum_pos_it >= current_read_pos + readlen - 50)//quality_thresh)
			break;
	
		if(current_read_pos >= *temp_cum_pos_it)
		{
			startposa = 0;
			matchlen = readlen - (current_read_pos - *temp_cum_pos_it);
			startposb = readlen -  matchlen;
		}
		else
		{
			startposb = 0;
			matchlen = readlen - (*temp_cum_pos_it - current_read_pos);
			startposa = readlen -  matchlen;
		}
		overlaps.push_back(-(double)matchlen/readlen+quality_thresh*(double)string_hamming(*reads_it,*temp_reads_it,startposa,startposb,matchlen)/matchlen);
		++temp_cum_pos_it;
		++temp_reads_it;
	}	
	if(overlaps.size() <= 50)
	...
	{
		std::nth_element(overlaps.begin(),overlaps.begin()+50,overlaps.end());
		hamming_limit = overlaps[50];
	}	
	while(true)
	{
		if(cum_pos_it == cumulative_pos.end())
			break;
		if(*cum_pos_it >= current_read_pos + readlen - 50)//quality_thresh)
			break;
		if(current_read_pos >= *cum_pos_it)
		{
			startposa = 0;
			matchlen = readlen - (current_read_pos - *cum_pos_it);
			startposb = readlen -  matchlen;
		}
		else
		{
			startposb = 0;
			matchlen = readlen - (*cum_pos_it - current_read_pos);
			startposa = readlen -  matchlen;
		}
//		read_weight = 1 - sqrt((double)string_hamming(*reads_it,*_reads_it,startposa,startposb,matchlen)/(matchlen*0.25));
		if(quality_thresh*(double)string_hamming(*reads_it,*_reads_it,startposa,startposb,matchlen)/matchlen -(double)matchlen/readlen<= hamming_limit)
			read_weight = 1;
		else
			read_weight = 0;
	//	read_weight = 1;
//		if(((double)string_hamming(*reads_it,*_reads_it,startposa,startposb,matchlen)/matchlen) <= quality_thresh)
//			read_weight = 1;
//		else
//			read_weight = 0;
		if(read_weight < 0)
			read_weight = 0;
//		if(num_reads_current_pos <= quality_thresh || cum_pos_it == cumulative_pos_it)
		for(int i = 0; i < matchlen; i++)
		{
			read_count[startposa+i][chartolong[(*_reads_it)[startposb+i]]] += read_weight*one_minus_p[(uint32_t)((*_quality_it)[startposb+i])-33];
		}
//		if(*(std::next(cum_pos_it)) > *cum_pos_it)
//			num_reads_current_pos = 1;
//		else
//			num_reads_current_pos++;
                ++cum_pos_it;
                ++_reads_it;
                ++_quality_it;
	}
	std::string corrected_read = generateRef(read_count);
	for(int j = 0; j < readlen; j++)

		if(read_count[j][chartolong[(*reads_it)[j]]] > alternate_thresh)///(read_count[j][0]+read_count[j][1]+read_count[j][2]+read_count[j][3]) > alternate_thresh)// && (read_count[j][0]+read_count[j][1]+read_count[j][2]+read_count[j][3]) > quality_thresh)// || (uint32_t)(*quality_it)[j]-33 > quality_thresh)
			corrected_read[j] = (*reads_it)[j];
...
void writecontig(std::vector<std::array<double,5>> &count, std::list<long> &pos, std::list<std::string> &reads, std::list<uint32_t> &order, std::list<char> &RC, std::list<std::string> &quality, std::ofstream& f_seq, std::ofstream& f_pos, std::ofstream& f_noise, std::ofstream& f_noisepos, std::ofstream& f_order, std::ofstream &f_RC, std::ofstream &f_order_N_pe, std::ofstream &f_flag_N, std::ofstream &f_quality, std::ofstream &f_quality_N, uint32_t list_size)
{
	...
	auto pos_it = pos.begin();
	*pos_it = 0;
	std::list<long> cumulative_pos(pos);//copy constructor
	auto cumulative_pos_it = cumulative_pos.begin(); 
	for(auto it = std::next(cumulative_pos_it); it != cumulative_pos.end(); ++it)
		*it = *it + *(std::prev(it));		
	auto reads_it = reads.begin();
	...
	double averaged_counts_allowed_flag;
	std::string corrected_read;
	for(;pos_it!=pos.end(); ++pos_it,++reads_it,++order_it,++RC_it,++quality_it,++cumulative_pos_it)
	{
		currentpos = prevpos + *pos_it;
		long prevj = 0;
		corrected_read = denoise_read(reads,cumulative_pos,quality,reads_it,cumulative_pos_it,quality_it,ref,count);
		for(long j = 0; j < readlen; j++)
		{
			if(corrected_read[j] != ref[currentpos+j])
			{
				f_noise<<enc_noise[ref[currentpos+j]][corrected_read[j]];
				c = j-prevj;
				f_noisepos<<c;
				prevj = j;
			}
...
std::vector<std::array<double,5>> buildcontig_new(std::list<std::string> &reads, std::list<long> &pos, std::list<std::string> &quality, std::uint32_t list_size)
{
	auto reads_it = reads.begin();
	auto quality_it = quality.begin();
	std::vector<std::array<double,5>> count(readlen,{0,0,0,0,0});
