...
for dataset in ERR204808_100_180
do
	...
	ref_fasta=$dirname"/ERR204808.genome.fasta"

	## Run Original File
	suffix="original_only_assembly"
	tmp_folder=$assembly_output_folder/$suffix
	#/usr/bin/time -v python $spades_tool -t 16 --only-assembler -1 $prefix"_1.fastq" -2 $prefix"_2.fastq" -o $tmp_folder | tee $log_folder/$suffix".spades_log"
	...
	suffix="using_Karect"
	#/usr/bin/time -v $karect_tool -correct -inputfile=$prefix"_1.fastq" -inputfile=$prefix"_2.fastq" -resultdir=$dirname -celltype=diploid -matchtype=hamming -threads=16 | tee $log_folder/$suffix".karect_log"
